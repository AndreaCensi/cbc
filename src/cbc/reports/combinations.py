...
def create_report_comb_stats(comb_id, tc_ids, alg_ids, deps):
    ...
    r.table('abs_rel_error_deg', caption='Final average relative error (deg)',
            **generic_table(tc_ids, alg_ids,
                            tablevar('rel_error_deg')))

    r.table('scaled_rel_error', caption='Final relative error (after scaling)',
            **generic_table(tc_ids, alg_ids,
                            tablevar('scaled_rel_error', '%.4f')))

    r.table('scaled_error', caption='Final abs error (after scaling+ortho.)',
            **generic_table(tc_ids, alg_ids,
                            tablevar('scaled_error', '%.4f')))

    r.table('scaled_rel_error_deg',
            caption='Final relative error (deg) (after scaling)',
            **generic_table(tc_ids, alg_ids,
                            tablevar('scaled_rel_error_deg', '%.4f')))
    r.table('scaled_error_deg',
            caption='Final abs error (deg) (after scaling+ortho.)',
            **generic_table(tc_ids, alg_ids,
                            tablevar('scaled_error_deg', '%.4f')))
    ...
    r.table('spearman_robust', caption='Spearman correlation (robust)',
            **generic_table(tc_ids, alg_ids,
                            tablevar('spearman_robust', '%.4f')))

    r.table('diameter', caption='Diameter of solution',
            **generic_table(tc_ids, alg_ids,
                            tablevar('diameter_deg', '%.1f')))

    r.table('phase', caption='Convergence phase',
            **generic_table(tc_ids, alg_ids,
                            tablevar('phase', '%s'), mark_lower=False))
    ...
    for tc_id in tc_ids:
        ...
        generic_iteration_plot(rtc, ftc, 'error_deg', plot_alg_ids,
                               variable('error_deg'),
        ...
        generic_iteration_plot(rtc, ftc, 'spearman', plot_alg_ids,
                               variable('spearman'),
                               caption='Spearman correlation')
        generic_iteration_plot(rtc, ftc, 'spearman_robust', plot_alg_ids,
                               variable('spearman_robust'),
                               caption='Spearman correlation (rob)')
        generic_iteration_plot(rtc, ftc, 'diameter', plot_alg_ids,
                               variable('diameter_deg'),
                               caption='Diameter of solution')

        generic_iteration_plot(rtc, ftc, 'robust', plot_alg_ids,
                               variable('robust'),
                               caption='robust')
